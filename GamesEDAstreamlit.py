...
st.write(
    "This streamlit contains analysis of scrapped data from Steam from 1997 to 2024"
)
...
st.write(
    f"The dates Range from {games['release_date'].min()} to {games['release_date'].max()}"
)
...
if graphic == "Graphs":
    ...
    x_col = st.selectbox(
        "Select Desired X Axis",
        options=["Years", "Operating System", "Languages", "Developers Are Publishers"],
    )
    y_col = st.selectbox(
        "Select Desired Y Axis",
        options=["Count", "Price", "Peak CCU", "Percent Positive"],
    )
    # create year graphs
    if x_col == "Years":
        ...
        if y_col == "Count":
            release_year_freq = (
                games["release_date"].dt.year.value_counts().sort_index()
            )
            # Plot the release year frequency using Pandas
            release_year_freq.sort_index().plot(
                kind="bar",
        ...
        if y_col == "Price":
            ...
            avg_price_by_year = (
                games.groupby("release_year", observed=False)["price"]
                .mean()
                .sort_index()
            )
        ...
        if y_col == "Peak CCU":
            avg_peak_ccu_by_year = (
                games.groupby("release_year", observed=False)["peak_ccu"]
                .mean()
                .sort_index()
            )

            # Plot the results using a bar graph
            avg_peak_ccu_by_year.plot(
                kind="bar",
        ...
        if y_col == "Percent Positive":
            ...
            avg_percent_positive_by_year = (
                games.groupby("release_year", observed=False)["percent_positive"]
                .mean()
                .sort_index()
            )

            # Plot the results using a bar graph
            avg_percent_positive_by_year.plot(
                kind="bar",
    ...
    if x_col == "Operating System":
        ...
        if y_col == "Price":
            ...
            average_prices = [
                games["price"].loc[games[x] == True].mean() for x in op_sys
            ]
        ...
        if y_col == "Peak CCU":
            ...
            average_ccu = [
                games["peak_ccu"].loc[games[x] == True].mean() for x in op_sys
            ]
        ...
        if y_col == "Percent Positive":
            ...
            average_pospct = [
                games["percent_positive"].loc[games[x] == True].mean() for x in op_sys
            ]
    ...
    if x_col == "Languages":
        ...
        if y_col == "Percent Positive":
            ...
            average_pospct = [
                games["percent_positive"].loc[games[x] == True].mean() for x in lang
            ]
    ...
    if x_col == "Developers Are Publishers":
        ...
        if y_col == "Price":
            # Group by year, aggregate price average
            avg_price_by_dev_pub = (
                games.groupby("dev_pub", observed=False)["price"].mean().sort_index()
            )
            # Plot the results using a bar graph
            avg_price_by_dev_pub.plot(
                kind="bar",
        ...
        if y_col == "Peak CCU":
            avg_peak_ccu_by_dev_pub = (
                games.groupby("dev_pub", observed=False)["peak_ccu"].mean().sort_index()
            )

            # Plot the results using a bar graph
            avg_peak_ccu_by_dev_pub.plot(
                kind="bar",
        ...
        if y_col == "Percent Positive":
            avg_percent_positive_by_dev_pub = (
                games.groupby("dev_pub", observed=False)["percent_positive"]
                .mean()
                .sort_index()
            )

            # Plot the results using a bar graph
            avg_percent_positive_by_dev_pub.plot(
                kind="bar",
...
if graphic == "Tables":
    ...
    y_col = st.selectbox(
        "Select Desired Columns", options=["Price", "Peak CCU", "Percent Positive"]
    )
    ...
    if x_col == "Operating System":
        ...
        if y_col == "Price":
            # Plot average price by Operating System
            op_prices = games.groupby(op_sys, observed=False)[["price"]].agg(
                {"price": ["describe"]}
            )
            st.write(op_prices)
        ...
        if y_col == "Peak CCU":
            # Plot average peak_ccu by Operating System
            op_ccu = games.groupby(op_sys, observed=False)[["peak_ccu"]].agg(
                {"peak_ccu": ["describe"]}
            )
            st.write(op_ccu)
        ...
        if y_col == "Percent Positive":
            # Plot average percent positive review by Operating System
            op_pospct = games.groupby(op_sys, observed=False)[["percent_positive"]].agg(
                {"percent_positive": ["describe"]}
            )
            st.write(op_pospct)
    ...
    if x_col == "Languages":
        ...
        if y_col == "Price":
            # Plot average price by language
            lang_prices = games.groupby(lang, observed=False)[["price"]].agg(
                {"price": ["describe"]}
            )
        ...
        if y_col == "Peak CCU":
            # Plot average peak_ccu by language
            lang_ccu = games.groupby(lang, observed=False)[["peak_ccu"]].agg(
                {"peak_ccu": ["describe"]}
            )
            st.write(lang_ccu)
        ...
        if y_col == "Percent Positive":
            # Plot average percent positive review by language
            lang_pospct = games.groupby(lang, observed=False)[["percent_positive"]].agg(
                {"percent_positive": ["describe"]}
            )
    ...
    if x_col == "Developers Are Publishers":
        ...
        if y_col == "Price":
            # Group by year, aggregate price average
            avg_price_by_dev_pub = (
                games.groupby("dev_pub", observed=False)[["price"]]
                .agg({"price": "describe"})
                .sort_index()
            )
        ...
        if y_col == "Peak CCU":
            avg_peak_ccu_by_dev_pub = (
                games.groupby("dev_pub", observed=False)[["peak_ccu"]]
                .agg({"peak_ccu": "describe"})
                .sort_index()
            )
